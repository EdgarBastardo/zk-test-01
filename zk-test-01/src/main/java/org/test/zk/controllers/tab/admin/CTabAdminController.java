...
public class CTabAdminController extends SelectorComposer<Component> {
	...
	public void onClickbuttonPersonManager (Event event) {
		if (controllerLogger != null)
			controllerLogger.logMessage( "1" , CLanguage.translateIf( controllerLanguage, "Button Person Manager" ) );

		Map<String, Object> arg = new HashMap<String, Object>();
        arg.put( "callerComponent", event.getTarget() ); // le paso el boton que llamo al evento event.getTarget()
        Window win = (Window) Executions.createComponents("/views/person/manager/manager.zul", null, arg); //llama a Dialog
        win.doModal();
